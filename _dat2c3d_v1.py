...
2018-May-04 J. Heath Harwood script was modified to accept HF *.dat_mod files
...
mgrsDir = r'D:\2018_NCMP_EC\processing\_camera\_mgrs_grids\WGS84_GEO'
...
def eos2shp(eoFileList):
    ...
    for eo in eoFiles:
        ...
        eoPath = os.getcwd()
        print eoPath
        dsName = eoPath.split('\\')[-1]
        print dsName
        global eoName
        eoName = eo
        print eoName
        # Establish file names
        eoShp = eoPath + '\\' + '_' + dsName + '_' + eoName.strip('dat_mod') + 'shp'
        eoPoly = eoPath + '\\' + '_' + dsName + '_' +eoName.split('.')[0] + '_eoMBGPoly' + '.shp'
        eoBufferShp = eoPath + '\\' + '_' + dsName + '_' +eoName.split('.')[0] + '_eoBuff500mPoly' + '.shp'
        eoPrj = eoPath + '\\' + '_' + dsName + '_' + eoName.strip('dat_mod') + 'prj'
        eoTxt = eoPath + '\\' + '_' + dsName + '_' + eoName.strip('.dat_mod') + '_for_c3d.txt'
        eoAPS = eoPath + '\\' + '_' + dsName + '_' + eoName.strip('.dat_mod') + '_for_photoscan.txt'
        print '     The Phase One EO File is:  ' + eo
        ...
        eoData = np.genfromtxt(eoFile, dtype=None,
                                  usecols=(1, 0, 2, 3, 4, 5, 6, 7, 10, 11),
        ...
        for grid in gridFiles:
            ...
            eoGridShp = eoPath + '\\' + '_' +  dsName + '_' + eoName.split('.')[0] + '_' + grid[0:12] + '.shp'
            zone = grid[9:11]
            print curGrid
            #print eoGridShp
    ...
    for i in range(layerBuff.GetFeatureCount()):
        ...
        for i in range(layerGrid.GetFeatureCount()):
            ...
            if geometry2.Intersects(geometry1):
                ...
                dstfeature.SetField("Shape_Leng", attShape_Leng)
                #dstfeature.SetField("Shape_Length", attShape_Le_1)
...
if __name__ == '__main__':
    eoFileList = easygui.fileopenbox(msg="Select HydroFusion *.dat_mod Files", title= "EO Selection", default='*', filetypes=".dat", multiple=True)
    eos2shp(eoFileList)
    os.system(r'C:\Anaconda2\python.exe _PhaseOne_quickThumbs64bit_20180503.py')
