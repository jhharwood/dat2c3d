...
mgrsDir = r'D:\_dev\_programming\_python\dat2c3d\_mgrs_grids\WGS84_GEO'
...
def eos2shp(eoFileList):
    ...
    for eo in eoFiles:
        ...
        eoName = eo
        print eoName
        eoShp = eoPath + '\\' + eoName.strip('dat') + 'shp'
        eoPoly = eoPath + '\\' + eoName.split('.')[0] + '_eoMBGPoly' + '.shp'
        eoBufferShp = eoPath + '\\' + eoName.split('.')[0] + '_eoBuff500mPoly' + '.shp'
        eoPrj = eoPath + '\\' + eoName.strip('dat') + 'prj'
        eoTxt = eoPath + '\\' + eoName.strip('.dat') + '_for_c3d.txt'
        eoAPS = eoPath + '\\' + eoName.strip('.dat') + '_for_photoscan.txt'
...
if __name__ == '__main__':
    eoFileList = easygui.fileopenbox(msg="Select HydroFusion *.dat Files", title= "EO Selection", default='*', filetypes=".dat", multiple=True)
    eos2shp(eoFileList)
